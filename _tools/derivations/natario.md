...
The Natário warp drive is written in two coordinate systems, the Cartesian flow chart and the plane symmetric chart.
...
The Natário drive is a construction rather than a solution.
...
That is the honest way to read every warp drive, and the arithmetic matters because the cost comes out negative and stays negative no matter which flow is chosen.

Two things follow for a general flow that Alcubierre's single flow cannot settle.
By Step 4 the expansion of the drive is one number, the divergence of the flow, and Natário's condition is the vanishing of exactly that number.
With the divergence left free, the energy density of Step 11 is a difference of two squares, of which Natário's condition removes the positive one.
...
It is the convention on which ordinary matter has a positive energy density, which is what makes the negative one of Step 11 a statement about the drive rather than about a sign convention.
...
A partial derivative with a time index is taken with respect to the same chart coordinate,
...
The dots in the geodesic equations are velocities of that same chart, so $\dot{t}$ means $d(ct)/d\lambda$, and that reading is the one on which every term balances in Step 23.

The two coordinate systems are not two charts of one spacetime.
...
The pp wave is written the same way, its exact plane wave chart being the subcase of its Brinkmann chart with a quadratic profile, and for the same reason: the general member is where the definition lives and the special member is where the curvature closes.
...
which is the line element of the Cartesian flow chart.
This is the 3+1 form with lapse one, flat spatial slices and shift $-\vec{V}$, and it is the only structure a warp drive has.
Nothing is assumed about $\vec{V}$ until Natário's condition in Step 4.

The flow has a direct physical reading.
...
as the metric of Step 3 confirms.
...
His own profile takes $f$ to be a function of the distance $r_s = \sqrt{\left(x - x_s(t)\right)^2 + y^2 + z^2}$ from the ship, and Alcubierre's drive in Step 12 leaves $f$ an arbitrary function of all four coordinates.
...
with $V_i$ standing for $u$, $v$ and $w$ in turn.
Every one of them is dimensionless, as a chart component must be.

The determinant is worth a line of its own.
The matrix has $\delta_{ij}$ in its spatial corner, so its determinant is the Schur complement $\det(\delta_{ij})\left(g_{tt} - g_{ti}\delta^{ij}g_{jt}\right)$, which is
...
Natário's condition is about the three volume carried along the flow, computed in Step 4, not about this determinant.

The inverse is

$$g^{tt} = -1, \qquad g^{ti} = -\frac{V^i}{c}, \qquad g^{ij} = \delta^{ij} - \frac{V^iV^j}{c^2},$$

and it is checked in one line:
...
A divergence free field is the velocity field of an incompressible flow, so space slides around the ship like water around a stone with no parcel compressed on the way past.
...
which is the expression Alcubierre's drive has for $\nabla_\mu n^\mu$.
...
is the object every curvature tensor of the flow is built from, and Natário's condition is exactly that its trace vanish while its trace free part is left alone.
Nothing forces the trace free part to be small, and the negative energy of Step 11 is the price of that.
...
Writing $\beta^i = V^i/c$ for the dimensionless flow, the symbols are
...
Three of these say something about the flow.
...
Two components check the pattern.
For $\Gamma^t{}_{xy}$ the formula gives $\theta_{xy}/c = \left(\partial_yu + \partial_xv\right)/2c$, which is that symbol written out.
For $\Gamma^x{}_{yz}$ it gives $\beta^x\theta_{yz}/c = u\left(\partial_zv + \partial_yw\right)/2c^2$, which is also that symbol written out.
All forty symbols are written out in this way, because the checker reads symbol by symbol and has no way to be told what $\theta_{ij}$ means.

---

## Step 6. The lowered Christoffel symbols

Lowering with $g_{\mu\alpha}$ mixes the time and space rows, since $g_{ti}$ does not vanish, and the result is written out in full.
...
The lowered symbols have no compact formula, and the check they satisfy is that they are symmetric in their last two indices and reproduce the metric under

$$\partial_\rho g_{\mu\nu} = \Gamma_{\mu\nu\rho} + \Gamma_{\nu\mu\rho},$$

which sympy confirms component by component.
...
Written as two fractions, $\Gamma_{txy}$ with the minus sign taken off its front was character for character the expression for $\Gamma_{ytx} = \left(\partial_yu - \partial_xv\right)/2c$, and since a value is merged with its negation by that leading minus alone, the two collapsed into the one line $\Gamma_{txy} = -\Gamma_{ytx}$, which is false.
The rule of `_tools/README.md`, that a value be written so that a minus sign in front of the whole expression negates it, is what prevents that, and the rank four tensors of Step 9 rely on the same rule.
Every such merge in this chart has been checked back through the reader in `verify_metrics.py`: each of the 359 merged lines for the five tensors is a true identity between the components it names.
...
which the checker compares with $\theta$ written out in its three terms.
...
The Ricci tensor has sixteen nonzero components in each of its three index positions, and the whole of the work is done by one line of Step 3.
...
That one fact orders the three index positions by length.
The mixed form is the smallest of the three, since its whole time row is four terms wide, and the form with both indices down carries the longest Ricci component, since a lower time index drags the spatial components along with the flow.

The spatial components are the short ones, and they close in the rate of strain of Step 4:
...
which is that component written out, and the other five follow from the formula by permuting the letters.

The mixed time row is the momentum constraint with nothing else in it:

$$R^t{}_i = -n^\alpha R_{\alpha i} = \frac{\partial_i\theta - \nabla^2V_i}{2c},$$

so $R^t{}_x = \left(\partial_x\partial_yv + \partial_x\partial_zw - \partial_y^2u - \partial_z^2u\right)/2c$ has four terms.
The Einstein tensor agrees with it there, $G^t{}_i = R^t{}_i$, because the trace term carries $\delta^t{}_i = 0$, and the same expression is the piece of $G^{ti}$ in Step 10 that is not $G^{tt}$ carried along by the flow.
...
which is why $R_{tt}$ is the longest Ricci component: it carries the whole spatial part twice contracted with the flow.
...
And substituting the one component flow $\vec{V} = c\,v_sf\,\partial_x$ into every component in all three index positions reproduces, term for term, the Ricci tensor of Alcubierre's drive in the same chart and the same three index positions, which was derived independently of these.

On Natário's condition the expansion drops out of all of it: $R^{tt} = -\theta_{ij}\theta^{ij}/c^2$, the momentum constraint becomes $-\nabla^2V_i/2c$, and the spatial part loses its $\theta\,\theta_{ij}$ term.
...
Both it and the Weyl tensor are computed in full, each with the first index up and with every index down.
Their size is the cost of an arbitrary flow.
...
The two tensors in their two index positions are 672 components, 18738 terms and 642 kilobytes of LaTeX, four times every other expression of the spacetime put together, and the longest single component of them runs to 5005 characters, several times the width of a page.
The plane symmetric chart of Step 14 carries the same geometry in a few lines; this one holds the general statement, one component at a time.

The antisymmetries are what make those components collect by value rather than merely run long.
A value and its opposite are written as one expression and that expression with a minus sign in front, which lets the components of a tensor be collected by value: the 144 components of either lowered tensor collect on 21 lines, while the 192 of the mixed Riemann tensor take 96 and those of the mixed Weyl tensor 93, since the mixed form has only the antisymmetry in its last pair to collect on.
The rule is stated in `_tools/README.md`, with the Morris-Thorne wormhole as its worked example.

The Weyl tensor here is not a copy of the Riemann tensor anywhere.
Neither chart is a vacuum, so the trace terms of Step 21's definition do not vanish, and in this chart the two tensors are nonzero in the same 144 slots with every index down and disagree in every one of them.
That is the arithmetic behind the warning in `_tools/derivations/weyl.md`, and it is why the two tensors in their two index positions are four separate computations and not two.
...
In that frame the three parts of the Riemann tensor are the three equations of the 3+1 split, and each of them is short.
...
Together they recover Step 8: the spatial Ricci component is $R_{ij} = \delta^{kl}R_{kilj} - R_{0i0j}$, which is the Gauss part traced minus the Ricci equation part, the same formula.
...
Gathering the three parts,

$$K = R_{ABCD}R^{ABCD} = 4\sum_{ij}\left(R_{0i0j}\right)^2 - 4\sum_{ijk}\left(R_{0ijk}\right)^2 + \sum_{ijkl}\left(R_{ijkl}\right)^2,$$

with the two factors of four counting the index positions a component with two zeros and a component with one zero can each sit in.
Written out, it carries a 4 in front of each square whose two index pairs are the same and an 8 in front of each square that the sum reaches twice.
Expanded flat it is 498 monomials and 25731 characters; written this way it is 4542, and this is the form the checker compares.
...
On the plane symmetric flow $\vec{V} = u(t,y,z)\,\partial_x$ it collapses to the Kretschmann scalar of Step 20 in that chart.
On Alcubierre's flow $\vec{V} = c\,v_sf\,\partial_x$ it collapses to the Kretschmann scalar of Alcubierre's drive in its own chart.
And `_tools/derivations/verify_metrics.py` rebuilds it from the line element, with its own Riemann tensor and its own contraction by the inverse metric, and agrees, once it is given the budget of Step 24.

Three readings.
The Codazzi part enters with a minus sign, so $K$ is not positive definite for any flow, which is the general form of the remark in Step 20 about the plane member.
...
With both indices up, the pieces of the Einstein tensor of the general flow are the constraint equations of the 3+1 split and can be read off rather than dug out.
...
which written out in the components of the flow is
...
with $S^{ij}$ the part that is genuinely new, written out term by term for the checker.
Every one of the ten independent components is one of these three objects plus the flow carrying the others along, which is why each of them repeats the expression for $G^{tt}$ inside it.
...
so it is already computed in Step 10:
...
This is the central result for the Natário drive, and the way it arrives matters.
...
On Alcubierre's own field, in Step 12, this sharpens from an inequality into an identity, and the expansion cancels out of the energy density exactly.
In that sense the expansion was scenery.

The null energy condition, which is weaker than the weak one and so could in principle survive it, fails as well.
An explicit null vector in the plane symmetric chart, where the arithmetic closes, gives the same sign in Step 19.
...
The comparison can be made exact, because Alcubierre's drive is written in the same chart and the same conventions.
...
In the notation of Step 2 that is the flow

$$\vec{V} = c\,v_s f\,\partial_x,$$

a flow with one component which does depend on the coordinate it points along.
Its expansion is $\theta = \partial_xu = c\,v_s\,\partial_xf$, which is the expression Alcubierre's drive has for $\nabla_\mu n^\mu$, so the two calculations agree on the number that separates the drives in Step 4.
...
which is the energy density of Alcubierre's drive, and $G^{tt}$ and the Ricci scalar of Step 7 reproduce the corresponding components of Alcubierre's drive term for term when the same substitution is made.
Two derivations made independently agree, which is worth more than either of them alone.
...
which is the first geodesic equation, written out for the checker with $\theta_{ij}$ in its six terms.
...
That is the sense in which a ship at rest in the drive feels nothing, and it follows from the geodesic equations themselves rather than being asserted.
...
With three free functions of four coordinates the Riemann and Weyl tensors fill 672 slots across their two index positions each, by the count of Step 9, and every one of them is computed.
They are not readable, so the same geometry is written again in the member of the family where it closes.
...
So this is a Natário drive for every profile $u$ whatever, with no condition left to impose, and every component in this chart is a component of Natário's drive rather than of a general flow.
The line element is
...
In Step 12 the Alcubierre drive is the one component flow $\vec{V} = c\,v_sf\,\partial_x$, and Natário's condition on a flow with one component is the single requirement that it not depend on $x$, which for Alcubierre's flow reads $\partial_xf = 0$.
So the plane symmetric chart is the divergence free member of the Alcubierre family: Alcubierre's drive leaves the shape function arbitrary, and this chart imposes the one condition.
The two drives share the line element and the conventions, so the plane symmetric components of Steps 15 to 22 can be checked against the corresponding ones in `_tools/derivations/alcubierre.md`.
...
with every other component zero.
...
For instance Step 5 gives $\Gamma^y{}_{tx} = \beta^y\Gamma^t{}_{tx} + \omega_{yx}/c$, and with $\beta^y = 0$ that is $\partial_yu/2c$, which agrees.
...
The mixed form $R^\mu{}_{\nu\rho\sigma}$ is nonzero in a hundred slots and the lowered form in as many, since each independent value fills several of them through the antisymmetries.

Two of these are worth a remark.
The purely spatial part is the Gauss equation with nothing else in it.
...
The curvature of the spatial part is therefore the shear of the flow squared, with a minus sign, and it is a statement about how the slice sits in spacetime rather than about the slice, which is flat.
And $R_{yzyz}$ vanishes, which Step 21 needs.

---

## Step 18. The plane symmetric Ricci tensor and Ricci scalar

Contracting on the first index, $R_{\mu\nu} = R^\alpha{}_{\mu\alpha\nu}$, gives the Ricci tensor, of which the three components that carry the argument are
...
which is exactly the general result of Step 7 on Natário's condition:

$$\theta_{ij}\theta^{ij} = 2\left(\theta_{xy}^2 + \theta_{xz}^2\right) = \frac{\left(\partial_yu\right)^2 + \left(\partial_zu\right)^2}{2},$$

and $R = \theta_{ij}\theta^{ij}/c^2$ as it must be by Step 7.
The general formula and the special chart agree, and this is the one place the two coordinate systems can be checked against each other.

---

## Step 19. The plane symmetric Einstein tensor, the matter and the energy conditions

The components of the Einstein tensor are short enough here to give in all three index positions.
...
with $G^{tx}$ and $G^{xx}$ carrying these along with the flow as in Step 10.
...
That is the central claim about the Natário drive, holding on an exact solution with no small parameter anywhere in it.
...
in which every profile dependence is transverse.
...
Three things follow.
Where the profile is flat, inside the corridor and outside it, $K$ vanishes and the geometry is flat, which is the statement of Step 14 about the Christoffel symbols.
...
The obstructions to building one are the negative energy of Step 19 and the horizons of a superluminal drive, not a curvature that runs away.
...
in four dimensions, with the traces built from the same Ricci tensor, contracted on the first index.
...
The computation follows the working of `_tools/derivations/weyl.md` on exactly this point.

The difference is visible in slots where the two disagree about whether anything is happening at all.
By Step 17, $R_{yzyz} = 0$.
...
Copying Riemann into the Weyl tensor would have given zero in both places and would have been wrong in both.

The purely spatial part of Weyl,
...
each with every term written out.
...
## Step 23. Every equation is dimensionally consistent

The declarations are $[t] = T$, $[x] = [y] = [z] = L$ and $[u] = [v] = [w] = L/T$, which is `DIMENSIONS` in `verify_metrics.py` for both systems.
Since the chart is $x^0 = ct$, a chart component with $p$ upper time indices and $q$ lower ones carries the bare component times $c^{p-q}$, and a $\partial_t$ written in any component is $c^{-1}\partial/\partial t$.
...
The three index positions of its Ricci and Einstein tensors carry $1/L^2$ slot by slot once the index weights of $x^0 = ct$ are put in: $R^{tt}$ is the bare component times $c^2$, $R^t{}_x$ times $c^0$ and $R_{tt}$ times $c^{-2}$, which is what makes $\left[\partial_t\theta_{ij}/c\right]$ and $\left[\theta_{ij}\theta^{ij}/c^2\right]$ land on the same $1/L^2$ in every one of them.
...
The dimensional pass of `verify_metrics.py` makes this check over every term of every expression and reports nothing for either system.

---

## Step 24. The components, and what the checker did

The Cartesian flow chart carries the full set for an arbitrary flow: the line element, the metric and its inverse, the Christoffel symbols with the first index up and with every index down, the Riemann tensor in the same two index positions, the Ricci tensor in all three index positions, the Ricci scalar, the Kretschmann scalar, the Einstein tensor in all three index positions, the Weyl tensor in two, and the four geodesic equations.
The Riemann and Weyl tensors cost, by the count of Step 9, 672 rank four components across their two index positions each, 18738 terms, 642 kilobytes of LaTeX, and a longest component of 5005 characters.
Written out, the spacetime comes to 1.2 megabytes, five times the largest other spacetime written out the same way, and the reason is the three free functions and nothing else.
The natural index position of the Einstein tensor is both indices up, where its three pieces are the constraints of the 3+1 split; the field equations and the trace are written in the other two index positions.

The plane symmetric chart carries the same full set: line element, metric, inverse, the Christoffel symbols and the Riemann tensor in two index positions each, the Ricci tensor in all three, the Ricci scalar, the Kretschmann scalar, the Einstein tensor in all three, the Weyl tensor in two, and the geodesics.

`verify_metrics.py` rebuilds all of it from the two line elements and compares it symbol by symbol, and requires every other component to vanish.
The plane symmetric chart takes about twenty seconds and reports no disagreement and nothing unchecked.
The Cartesian chart takes seven and a half minutes and reports no disagreement in any of its 896 expressions, but it needs `--budget 400` to report nothing unchecked.
Its Riemann tensor costs the script 41 seconds and its Weyl tensor 55, both index positions of each included; the Kretschmann scalar costs 182, because the script reaches that scalar by raising all four indices of the coordinate Riemann tensor and simplifying each of the 256 components rather than by the frame route of Step 9.
At the default budget of 120 seconds that one comparison comes back UNCHECKED, with the reason given, which is the same accounting `_tools/README.md` gives for Kerr on a larger scale.
...
The value itself is the one derived in Step 9 and checked there three further ways.
