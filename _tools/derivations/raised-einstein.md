...
The Einstein tensor of Godel's universe and of the Lanczos-van Stockum dust, with both indices up, is recomputed from the tensor with both indices down.

The doubly raised Einstein tensor of these two rotating dust solutions disagreed with sympy in eight components, four in each, in the audit of `_tools/derivations/audit-2026-09-18.md`.
In both spacetimes sympy agreed with $G_{\mu\nu}$ and with $G^\mu{}_\nu$ and disagreed with $G^{\mu\nu}$, and in both the old $uu$ components were not symmetric.
A symmetric tensor stays symmetric under raising with a symmetric metric, so the asymmetry alone was proof that the old values were not the raised $ll$ values, whatever else they were.

The correction does not adjust the old values until they become symmetric.
The $uu$ components of each spacetime are recomputed from that spacetime's own $G_{\mu\nu}$ and its own $g^{\mu\nu}$, by

$$G^{\mu\nu} = g^{\mu\alpha}g^{\nu\beta}G_{\alpha\beta},$$

and the result is then checked three ways: against the spacetime's own $G^\mu{}_\nu$, against the invariant form the source of the field has, and against the spacetime's own Ricci scalar through the trace.
The $ll$ and $ul$ components were already right and are unchanged.
...
The raising that gives the $uu$ components, $G^{\mu\nu} = G^\mu{}_\alpha g^{\alpha\nu}$, is the matrix product
...
It cannot be written in index notation at all: $g^{\mu\alpha}G^\alpha{}_\nu$ repeats $\alpha$ upstairs twice and so is not a contraction, which is exactly the error the notation exists to forbid and the reason it is stated as a matrix product instead.
The slip is hard to catch once made, for the reason of Step 4: the two orders have the same trace, so the one cheap test a reader would apply passes on both.

---

## Step 1. Conventions

The signature is $(-,+,+,+)$ and the Ricci tensor is the standard contraction $R_{\mu\nu} = R^\alpha{}_{\mu\alpha\nu}$, the contraction in use since 2026-09-18.
The Einstein tensor is $G_{\mu\nu} = R_{\mu\nu} - \tfrac{1}{2}Rg_{\mu\nu}$.

Neither spacetime needs a factor of $c$ carried anywhere, because neither chart has a coordinate that carries dimensions of time.
...
So no component picks up a $c$ on raising a time index, and raising a time index is exactly where a factor of $c$ would appear if the chart had one.
...
That arithmetic catches, in Step 3, the two van Stockum components the dimensional pass flagged.
...
so in the order $(t,x,y,z)$ the metric and its inverse are
...
Only the $(t,y)$ submatrix is nontrivial, and there

$$\det\begin{pmatrix} -1 & -e^x \\ -e^x & -\tfrac{1}{2}e^{2x}\end{pmatrix} = \frac{e^{2x}}{2} - e^{2x} = -\frac{e^{2x}}{2},$$

so the inverse of that submatrix is $\dfrac{2\omega^2}{-e^{2x}/2}\begin{pmatrix} -\tfrac{1}{2}e^{2x} & e^x \\ e^x & -1\end{pmatrix} = 2\omega^2\begin{pmatrix} 1 & -2e^{-x} \\ -2e^{-x} & 2e^{-2x}\end{pmatrix}$, which agrees with the inverse metric.
The inverse metric does invert the metric, so the raising is done with Godel's own inverse metric and not with a repaired one.

### The lowered Einstein tensor

The Ricci tensor and scalar are $R_{tt} = 1$, $R_{ty} = R_{yt} = e^x$, $R_{yy} = e^{2x}$ and $R = -2\omega^2$, and from $G_{\mu\nu} = R_{\mu\nu} - \tfrac{1}{2}Rg_{\mu\nu}$,
...
which agrees exactly with the $ll$ components as they stood.
Those components are the starting point and they do not change.

### Raising one index

Take it in two passes, so that the intermediate can be checked against the mixed components, which were already right.
...
The $ul$ components as they stood are $G^t{}_t = -\omega^2$, $G^t{}_y = -2e^x\omega^2$, $G^x{}_x = G^y{}_y = G^z{}_z = \omega^2$, with $G^y{}_t$ zero.
Every one of the six agrees, which is the first confirmation that the $ll$ components and the inverse metric being used are the right pair.
...
$G^{ty} = G^{yt} = -4\omega^4e^{-x}$, so the tensor is symmetric.
...
All four of the components in the $(t,y)$ submatrix were wrong, and all four are entries of $[g^{-1}][G]$:
...
which are the four old values term for term.
...
Godel's source is dust with a cosmological constant, and the check that the recomputed components are right and not merely symmetric is that they have the form the source has.
...
Every value, old and new, is a pure number times $\omega^4$, which is why the dimensional pass never flagged Godel: a dimensional check cannot see an error made in the coefficients alone.
...
The $(t,\phi)$ submatrix has determinant
...
That agrees with the inverse metric as it stood, so again the raising uses van Stockum's own inverse metric.

### The lowered Einstein tensor

The Ricci scalar is $R = 4e^{r^2/R^2}/R^2$ and, abbreviating $E = e^{r^2/R^2}$, the Ricci tensor has the components
...
which agrees with the $ll$ components as they stood: four nonzero components, all in the $(t,\phi)$ submatrix, and the $rr$ and $zz$ components cancelling exactly.
...
The $ul$ components as they stood are exactly $G^t{}_t = -4E/R^2$ and $G^t{}_\phi = -4Er^2/R^3$ and nothing else, so they agree.
...
has $tt$ component $4E/R^2$, $t\phi$ component $4Er^2/R^3$, $\phi\phi$ component $4Er^4/R^4$ and nothing else, which are the $ll$ components term for term.
So
...
The four old $uu$ components were three too many: the energy density of a comoving dust sits entirely in $G^{tt}$ when the indices are up, and the off-diagonal terms that appear with the indices down are there only because $u_\phi = -r^2/R$ does not vanish.
The checker requires every other component to vanish, so the three zeros are checked rather than assumed.
...
all four of which are the old values exactly.
The whole old matrix is the column $g^{\mu t}$ times the row $G^t{}_\nu$, which is why it is a rank one matrix that is not symmetric: $g^{\mu t}$ and $G^t{}_\nu$ are not proportional.
...
The old $G^{t\phi} = -4Er^2(r-R)(r+R)/R^5$ expands to two terms, $-4Er^4/R^5$ and $4Er^2/R^3$, each carrying $1/L$ rather than $1/L^3$, and the old $G^{\phi\phi} = 4Er^2/R^4$ carries $1/L^2$ rather than $1/L^4$.
...
Which two got flagged and which two did not follows from the diagnosis of the slip.
...
The old $G^{tt}$ carried $1/L^2$ correctly and was still the wrong number, which is the audit's point that the dimensional pass is a filter and not a superset.
...
Both old tensors pass it.
For Godel,

$$g_{\mu\nu}G^{\mu\nu}_{\text{old}} = 2\omega^2 = -R,$$

and for van Stockum,

$$g_{\mu\nu}G^{\mu\nu}_{\text{old}} = -\frac{4e^{r^2/R^2}}{R^2} = -R,$$

and the recomputed tensors give the same two numbers.
...
Both orders return $\operatorname{tr}[G] = G^\mu{}_\mu$, which is $-R$, and the $ul$ components they are both built from were correct.
The identity needs nothing of this spacetime, so raising the wrong index can never be caught by the trace in any spacetime.

The test that does separate the two orders is the one the audit used.
$[G][g^{-1}]$ is symmetric, because it is $[g^{-1}][G_{ll}][g^{-1}]$ with $[G_{ll}]$ and $[g^{-1}]$ both symmetric, while $[g^{-1}][G]$ is a product of two matrices that do not commute, and neither old tensor was symmetric.

## Step 5. What changed

In Godel's universe, in its Cartesian chart, $G^{tt}$ changed from $-2\omega^4$ to $6\omega^4$, $G^{ty}$ from $-8\omega^4\cosh x$ to $-4e^{-x}\omega^4$, $G^{yt}$ from $4e^{-x}\omega^4$ to $-4e^{-x}\omega^4$, and $G^{yy}$ from $4(2 + e^{-2x})\omega^4$ to $4e^{-2x}\omega^4$.
$G^{xx}$ and $G^{zz}$ were already $2\omega^4$ and are untouched.

In the Lanczos-van Stockum dust, in its cylindrical chart, $G^{tt}$ becomes $\dfrac{4e^{r^2/R^2}}{R^2}$, and $G^{t\phi}$, $G^{\phi t}$ and $G^{\phi\phi}$ are zero, so they are dropped and the checker holds them to zero.

Nothing else in either spacetime changed.
The Einstein tensors with both indices down and with one index up, the Ricci tensors, the Ricci scalars, the Christoffel symbols, the Riemann tensors, the Weyl tensors corrected earlier the same day, and the geodesics Godel gained the same day are all as they were.
