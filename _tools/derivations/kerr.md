...
We work out the Kerr black hole in its Boyer-Lindquist coordinate system.
...
Kerr has the longest connection of all the spacetimes: twenty independent Christoffel symbols with an upper index, twenty more with all three lowered, and thirteen independent components of the Riemann tensor.
...
They are found in Step 7, and squaring them gives the Kretschmann scalar in one line in Step 11.
...
For this spacetime the choice costs nothing, because both contractions vanish, by Step 8, and a tensor that is zero is zero with either sign in front of it.
The components are written the standard way regardless.

Factors of $c$ and $G$ are kept explicit.
Kerr is one of the two spacetimes that keep a mass as a mass rather than folding it into a length, so $M$ appears with Newton's constant beside it and the Schwarzschild radius of that mass, $r_s = 2GM/c^2$, is spelled out as $2GM/c^2$ in every component.
We use the shorthand $r_s$ in Steps 2 to 12, because carrying $2GM/c^2$ through eighty lines of algebra obscures the algebra; in Step 13 it is written out again.

The chart, here as for every other spacetime, is the one whose time coordinate is $x^0 = cT$.
In Schwarzschild the convention is plain: $g_{tt} = -(1-r_s/r)$ stands against a line element whose time term is $-(1-r_s/r)c^2dt^2$.
...
A scalar has no index at all and is therefore the same number in both charts, which is what lets us compute the Kretschmann scalar in Step 11 without thinking about the chart.

We work in the bare $(t,r,\theta,\phi)$ chart in Steps 2 to 12, where $g_{tt}$ carries a $c^2$, because that is where the computation is done.
The results are carried into the rescaled chart in Step 13, and the arithmetic there is the check that no factor of $c$ was dropped: every $c$ that appears in the bare tables of Steps 3 to 7 turns out to be exactly $c$ to the power of the number of lower $t$ indices, so the components in the rescaled chart are the same formulas with the $c$ powers struck out.
...
Three derivatives do most of the work,
...
which are real and distinct when $a < GM/c^2$, and the domain of the chart is $r > r_+$.
Neither root is a curvature singularity, by Step 12, so both are places the chart fails rather than places the spacetime does.
...
The numerator of $g^{tt}$ has a second form, the one we use, because it is the one that shows the function is not singular at the horizon by accident:
...
Raising gives $\Gamma^r{}_{rr} = \frac{\Delta}{\Sigma}\Gamma_{rrr} = \dfrac{r}{\Sigma} - \dfrac{2r-r_s}{2\Delta}$, which is the form we keep, because each piece of it is a logarithmic derivative of one of the two functions.
...
These are the ones computed on the way in Step 4, and they are markedly shorter, because lowering the first index removes the $\Delta$ from most denominators:
...
$\Gamma_{\mu\nu\rho}$ is symmetric in $\nu\rho$ by construction, and $\Gamma_{\mu\nu\rho} + \Gamma_{\nu\mu\rho} = \partial_\rho g_{\mu\nu}$; whenever $g_{\mu\nu}$ is independent of $x^\rho$ the two are exact negatives, which is what every such equality with a minus sign in it is saying.
Twenty independent values again, thirty two components.
...
That kills $R_{trt\phi}$, $R_{trr\theta}$, $R_{t\theta t\phi}$, $R_{t\theta r\theta}$, $R_{t\phi r\phi}$, $R_{t\phi\theta\phi}$, $R_{r\theta r\phi}$ and $R_{r\theta\theta\phi}$, and leaves thirteen.
...
That is a check on all thirteen, because it fails for almost any transcription error.

The thirteen take a closed form in Step 7.
A direct attack works on the one component that needs nothing but a derivative.
...
The power of $c$ on each line is the whole of the dimensional check of Step 13.
...
which is exactly the value reached by the long route in Step 6.
...
Where it does not vanish, as in $R^t{}_{tt\phi} = g^{tt}R_{tt t\phi} + g^{t\phi}R_{\phi tt\phi}$, the surviving term carries $g^{t\phi}$, which is itself proportional to $r_s$, so the mixed component comes out quadratic in the mass, with $G^2M^2$ in its numerator.
Four of the forty four are of that kind: $R^t{}_{tt\phi}$, $R^t{}_{tr\theta}$, $R^\phi{}_{\phi t\phi}$ and $R^\phi{}_{\phi r\theta}$, and they are the only curvature components of any of the spacetimes that are quadratic in the mass.

---

## Step 8. The Ricci tensor vanishes

Kerr is a vacuum solution, and the vanishing of its Ricci tensor earns the claim.
...
Both contractions being zero is what the field equations demand: $R_{\mu\nu} = 0$ is the vacuum equation, with no cosmological constant, and the vanishing of every Ricci component is the statement that Kerr solves it.
...
The Ricci scalar is written `R = 0`.
...
The Ricci tensor, the Einstein tensor and the Ricci scalar vanish for that reason and not for want of working, which is what the conventions of Kerr say in as many words.
...
The order of that argument matters and is the reason the equality is not a shortcut.
...
The verification script builds the Weyl tensor from the definition, forming the correction and trace terms from its own Riemann tensor and its own inverse metric, and compares the result against the stored values; it never reads the stored Riemann components when it checks the stored Weyl components.
By hand the Weyl tensor was computed the same way, from the definition in all two hundred and fifty six slots, and then found to agree with Riemann in all of them, rather than assumed to.
...
## Step 13. The same components in the rescaled chart

The components are taken in the chart $x^0 = ct$.
...
Every factor of $c$ in the bare tables of Step 3 is removed exactly, and no metric component in the rescaled chart carries one, which is the statement that the chart components of a metric are dimensionless in the sense the checker means.
...
$\Gamma^t{}_{r\phi}$ and $\Gamma^t{}_{\theta\phi}$ have one upper $t$ and gain a $c$, which cancels the $1/c$ they carry in Step 4.
...
The same rule applies to the lowered symbols of Step 5, where a symbol with $n$ lower $t$ indices carries $c^n$ and loses all of it.

The curvature.
Every one of the thirteen lines of Step 7 carries $c$ to the power of the number of its lower $t$ indices, so the lowered Riemann tensor in the rescaled chart is that table with the $c$ powers struck out, for example
...
The same observation, made quantitatively, is the dimensional check of Step 15.

Finally, $r_s$ becomes $2GM/c^2$ everywhere, so that $\mathcal{A}$ and $\mathcal{B}$ become
...
and $\Sigma$ and $\Delta$ are written out in full, because the checker's reader knows only the declared parameters, and those are $M$, $a$ and $G$.
...
with the dot a derivative with respect to an affine parameter $\lambda$ and the components those of the rescaled chart, so $\dot{t}$ means $d(ct)/d\lambda$ and the $\Gamma$ in it is the one of Step 13.
...
The same argument appears in Step 17 of `kasner.md`, and it is what pins the dot convention down without any algebra.
...
That an inner bracket must balance on its own is why the bracket is written $c^2r\Sigma + GM(\ldots)$ rather than $r\Sigma + \tfrac{GM}{c^2}(\ldots)$: both are correct, but only the first has every term of every sum carrying the same dimension without a hidden division.

The checker's dimensional pass does this for every term of every expression of Kerr, which is 412 components, four geodesic equations, one line element and one scalar, in well under a second.
...
- thirty two Christoffel symbols with an upper index and thirty two with all three lowered, from twenty independent values each, which is the longest connection of any spacetime;
...
- a vanishing Ricci tensor, a vanishing Einstein tensor and `R = 0`, by Step 8 and Step 9;
- the Kretschmann scalar of Step 11;
- four geodesic equations.

Adding the spacetime to the checker took one line in `DIMENSIONS` in `verify_metrics.py`,
...
which is the documented step for adding a spacetime, and which declares the one thing the script cannot read off the components: that $a$ is a length rather than a dimensionless spin parameter, and that $t$ is the coordinate the chart multiplies by $c$.
Kerr is the second spacetime after Vaidya to keep $G$ and a mass explicit rather than folding them into a length, and so the second to need a mass in the table at all.
No line in `PARAMETER_RELATIONS` is needed, because $M$, $a$ and $G$ are free: every component is an identity for all three, not an identity on a constraint surface.

The index names of every component have to be the coordinate names spelled exactly as the coordinates are, which for Kerr means `"\\theta"` and `"\\phi"` rather than `"theta"` and `"phi"`.
That matters because getting it wrong is quiet in one direction and loud in the other: `compare_block` reports every mis-spelled index as a disagreement, but the dimensional pass skips a component whose indices it cannot resolve, so a first draft of the Kerr components passed `--dimensions-only` while three hundred and twenty of them were not being weighed at all.
A clean dimensional pass on a new spacetime is only worth something once the sympy comparison has confirmed that the index names resolve.

## Step 17. What the checker managed, and what it could not

Kerr is by a wide margin the slowest system of all to check, and until 22 September 2026 the script never completed a full sympy pass over it.
The reason is `norm`, the routine that puts an expression into a form where a vanishing one is recognisably zero.
It calls sympy's general `simplify`, which is cheap on the diagonal metrics of the other spacetimes and very expensive here: the metric is not diagonal, so the Christoffel sums do not collapse, and every curvature component is a rational function of $r$, $\sin\theta$ and $\cos\theta$ with $\Sigma$ to a high power underneath.
...
A tensor the script cannot finish inside its budget is reported `UNCHECKED` with the reason rather than passed in silence, so no claim rests on the script's authority that the script did not establish.
What was established instead, and what stands behind every component:
...
- Every stored value was compared against sympy by a separate cross check, which parsed each stored string with the script's own `Reader` and compared it against a tensor computed independently from the same line element, with the same chart weighting; it also confirmed that every other component vanishes. All 412 agreed.
- `check_system` itself was then run with only `norm` replaced, by a routine that reduces $\sin\theta$ and $\cos\theta$ to a canonical rational form rather than calling `simplify`. Everything else was the script: its reader, its metric, its Christoffel, Riemann, Ricci, Einstein and Weyl, and its component by component comparison. It reported zero disagreements on the metric, the inverse metric, the Christoffel symbols in both index positions, the Riemann tensor in both, the Ricci tensor in all three, the Einstein tensor in all three, the Weyl tensor in both and the Ricci scalar. It was stopped during the Kretschmann contraction, which the closed form of Step 11 covers by hand.

The honest summary is that the physics is verified and the tool is too slow to say so on its own.
Making `norm` fall back to a canonical trig reduction before reaching for `simplify` would bring Kerr inside the budget and would speed up the check of every spacetime, and that is the obvious next piece of work on the checker.

**Since 22 September 2026 the script says so on its own.**
`norm` now reads every expression as a rational function of its generators, reduces $\cos^2\theta$ by $1 - \sin^2\theta$, and keeps every denominator as a product of irreducible factors, so it never takes a polynomial gcd and never calls `simplify` on any spacetime; its docstring records the alternatives that were measured and why they lost.
...
The cross checks were not redone, and did not need to be: the script now reaches their verdict by itself.
