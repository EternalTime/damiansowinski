...
We work the Kerr-Newman spacetime in Boyer-Lindquist coordinates.
...
Kerr-Newman is worked in the same chart and the same shapes as Kerr in `_tools/derivations/kerr.md`, because the whole of the difference between the two spacetimes is one length.
Three facts organise the comparison, and each of them is derived rather than assumed.

The first is that the metric is Kerr's with the mass term $r_s r$ replaced by the mass function $\mathcal{M} = r_s r - r_Q^2$ (Step 2).
The second is that this replacement commutes with both derivatives the connection needs, because $\partial_r\mathcal{M} = r_s$ is what $\partial_r(r_sr)$ was and $\partial_\theta\mathcal{M} = 0$ is what it was, so the whole connection is Kerr's with the same replacement and nothing else (Step 4).
...
The field and its stress follow in Steps 6 to 8, the Weyl tensor separates from it in Step 11, and the Kretschmann scalar picks up exactly two new terms in Step 12.

Setting $r_Q = 0$ in any expression returns the corresponding Kerr expression.
...
Unlike in Kerr, the choice shows in the components, because the Ricci tensor of this spacetime is not zero.
The Ricci tensor on this convention is computed in Step 7 and carried into the Einstein tensor in Step 8, where it comes out with a positive energy density for the electromagnetic field, which is the check that the convention and the signature agree with each other.

Factors of $c$ and $G$ are kept explicit.
Kerr-Newman keeps a mass as a mass, as Kerr does, so $M$ appears with Newton's constant beside it and the Schwarzschild radius of that mass, $r_s = 2GM/c^2$, is spelled out as $2GM/c^2$ in every final component.
The charge is the one quantity that is not kept in its own units, because the checker's dimensional table is written in $L$, $T$ and $M$ and there is no base dimension in it for charge.
It is folded instead into the charge radius $r_Q$, exactly as it is for Reissner-Nordstrom, by
...
Through Steps 2 to 13 we use the shorthand $r_s$ and keep $r_Q$ as it stands; in Step 14 $2GM/c^2$ goes back in place of $r_s$.
...
Through Steps 2 to 13 we work in the bare $(t,r,\theta,\phi)$ chart, where $g_{tt}$ carries a $c^2$, because that is where the computation is done.
In Step 14 the results go over to the rescaled chart, and the arithmetic there is the check that no factor of $c$ was dropped.

---

## Step 2. The line element, and the four combinations the charge enters through
...
Setting $a = 0$ gives $\Sigma = r^2$ and $\Delta = r^2 - r_sr + r_Q^2$, and the line element collapses to Reissner-Nordstrom in the usual spherical chart.
...
Four derivatives do all the work:
...
Every quantity built from the metric by differentiating and multiplying is consequently the same function of $\left(r, \cos\theta, a, \mathcal{M}, r_s, \Sigma, \Delta\right)$ for both solutions, and the connection table of `kerr.md` serves for both.
...
which are real and distinct when $a^2 + r_Q^2 < G^2M^2/c^4$, and the domain of the chart is $r > r_+$.
Charge and spin enter that bound in the same way and on the same footing, which is the extremal condition of the solution; neither root is a curvature singularity, as Step 13 shows.
...
Two rearrangements recur throughout, and the components are written with them:
...
$\Gamma_{\mu\nu\rho}$ is symmetric in $\nu\rho$ by construction, and $\Gamma_{\mu\nu\rho} + \Gamma_{\nu\mu\rho} = \partial_\rho g_{\mu\nu}$; whenever $g_{\mu\nu}$ is independent of $x^\rho$ the two are exact negatives, which is what every equality with a minus sign says.
Ten of these thirty two components are equal up to sign to one of the others, which is why the lowered symbols are built from ten written values rather than twenty.
...
in the bare chart, together with the antisymmetric partners; the two factors of $c$ drop out in Step 14 along with every other one.
...
The pair $(U, V)$ is not an ad hoc device.
Write $z = r - ia\cos\theta$, as in `kerr.md` Step 7, so that $z\bar z = \Sigma$ and $\bar z^2 = \left(r^2-a^2\cos^2\theta\right) + 2iar\cos\theta$.
Then the bracket in the field strength is $-\mathrm{Re}\left[\bar z^2\left(U + iV\right)\right]$, so the electromagnetic field and the curvature of Step 10 are built on the same complex structure and the same two principal null directions.
...
That is the entire role of the charge in the gravitational field: it enters as the length $r_Q$ and in no other way, which is why no component mentions $Q$, $\epsilon_0$ or $\mu_0$ at all.
...
Reissner-Nordstrom has $R^{\hat\mu}{}_{\hat\nu} = \left(r_q^2/r^4\right)\mathrm{diag}(-1,-1,1,1)$, which is this with $\Sigma \to r^2$.

Written out in the rescaled chart, the six nonzero components are
...
Every one of the eighteen Ricci components carries the factor $r_Q^2$, and every one of them vanishes when the charge does.
That is the whole of the difference between the Ricci tensor of Kerr-Newman and the vanishing one of Kerr.
...
The scale has a direct physical meaning.
...
$R = g^{\mu\nu}R_{\mu\nu} = 0$, which follows from the trace of the field equations in Step 7 rather than from contracting the components, and which the contraction confirms: the frame form $\mathcal{E}\,\mathrm{diag}(-1,-1,1,1)$ has trace zero by inspection.
The Ricci scalar vanishes here for a different reason than it does in Kerr.
...
The Einstein tensor in its three index positions therefore has the same eighteen values as the Ricci tensor, and that equality is a fact about this solution rather than a copy: it is what a trace free source does.
...
It is a check on the whole Riemann tensor and it fails for almost any transcription error.

What is new is that the Riemann tensor is no longer the Weyl tensor.
The difference is fixed by the Ricci tensor of Step 7 through the definition of Step 11, and because that Ricci tensor is $\mathcal{E}\,\mathrm{diag}(1,-1,1,1)$ in the frame, the difference is as simple as a difference of two curvature tensors ever gets:
...
Each Riemann component is written as its Weyl component plus its charge term, in that order, which separates the free gravitational field from the field of the charge sitting on top of it.
...
which are the two numerators of the definition.
...
The mixed components $C^\mu{}_{\nu\rho\sigma} = g^{\mu\alpha}C_{\alpha\nu\rho\sigma}$ follow by one contraction each, and there are more of them, eighty eight nonzero from thirty eight distinct values, because raising an index with a metric whose $(t,\phi)$ block is not diagonal turns one component into a mixture of two.
...
In Kerr those carry $G^2M^2$ in the numerator; here they are written as the product of the bracket $\mathcal{M}$ with the bracket $\Sigma^3\mathcal{A}$ or $\Sigma^3\mathcal{B}/(a\cos\theta)$, because the two factors no longer combine into a single power of the mass.
...
The correction, evaluated in Step 9, is $\mathcal{E}\left(U\otimes U + V\otimes V\right)$ and it is nonzero in six of the thirteen independent slots.
The Weyl tensor of Kerr-Newman is therefore not its Riemann tensor, where for Kerr the two are identical.

Copying the Riemann tensor into the Weyl tensor goes wrong exactly here, and that is the reason for the rule in `_tools/derivations/weyl.md` to compute the Weyl tensor rather than copy it.
Had the Weyl components been copied from the Riemann components, six of the thirteen independent values would have been wrong, and wrong by an amount that vanishes only in the limit that takes the solution back to Kerr.
The verification script builds the Weyl tensor from the definition, forming the correction from its own Riemann tensor and its own Ricci tensor, and checks each Weyl component against that, never against the Riemann components.
We computed the Weyl tensor by hand the same way, and recognised the closed form of Step 9 in the answer rather than assuming it at the start.
...
The electromagnetic field of Step 6 is aligned with the same two principal null directions, which is what a charged type D solution means and what makes the Newman-Janis trick work at all.
...
which with $r_s = 2GM/c^2$ is the Kretschmann scalar in full.
A scalar is the same number in every chart, so this expression needs no conversion in Step 14.

Three checks.
Setting $r_Q = 0$ leaves the first term, and $r^6-15r^4v^2+15r^2v^4-v^6 = \left(r^2-v^2\right)\left(\Sigma^2-16r^2v^2\right)$, which is the form of `kerr.md` Step 11.
Setting $a = 0$ gives $\Sigma = r^2$ and

$$K = \frac{12r_s^2r^6 - 48r_sr_Q^2r^5 + 56r_Q^4r^4}{r^{12}} = \frac{12r_s^2r^2 - 48r_sr_Q^2r + 56r_Q^4}{r^8},$$

which is exactly the Kretschmann scalar of Reissner-Nordstrom.
...
The rescaled chart is the chart $x^0 = ct$.
...
Every factor of $c$ in the bare components of Step 3 is removed exactly, and no metric component in the rescaled chart carries one.

The connection and the curvature.
Every symbol and every component in Steps 4, 5, 9 and 10 carries $c$ to the power of the number of its lower $t$ indices minus the number of its upper ones, so in the rescaled chart they are those formulas with the $c$ powers struck out.
That is true of the charge terms as well: $U_{tr} = c$ has one lower $t$ and $U^t{}_r = -\left(r^2+a^2\right)/(c\Delta)$ has one upper, which is why the first becomes $1$ and the second $-\left(r^2+a^2\right)/\Delta$, with no $c$ in either.
...
The parameters.
$r_s$ becomes $2GM/c^2$ everywhere, so the four combinations of Step 2 and Step 10 are written as
...
and $\Sigma$ and $\Delta$ are written out in full, because the checker's reader knows only the declared parameters, which are $M$, $a$, $G$ and $r_Q$.
The numerators of $\mathcal{A}$ and $\mathcal{B}$ are what appear, rather than $\mathcal{A}$ and $\mathcal{B}$ themselves, because the checker's reader knows no named functions; the powers of $\Sigma$ they carry are gathered into the denominator of each component.
Two identities between the four serve anyone checking a component by hand:
...
with the dot a derivative with respect to an affine parameter $\lambda$ and the components those of the rescaled chart, so $\dot{t}$ means $d(ct)/d\lambda$ and the $\Gamma$ in it is the rescaled one.
Each symbol with $\nu \ne \rho$ contributes twice, once from each ordering, which is the factor of two in front of the mixed terms.
Written with the combinations of Step 2 and the symbols of Step 4:
...
Written out in full, the same four equations have $r_s$ replaced by $2GM/c^2$ and the combinations expanded, and the $\dot{r}^2$ term of the $r$ equation becomes $r/\Sigma - \left(c^2r - GM\right)/\left(c^2\Delta\right)$, which is the same number written so that each bracket balances dimensionally on its own.
...
In this spacetime, unlike in Kerr, the distinction between the two motions is a real one.
...
The four combinations are each a sum, so each has to balance internally, and that is where the dimension declared for the charge radius earns its keep.
...
Every one of them would fail if $r_Q$ were taken to be dimensionless, which the script cannot read off the line element and which the Kerr-Newman line in `DIMENSIONS` declares.

Take the Ricci tensor, which vanishes for Kerr.
...
The same argument appears in `kasner.md` Step 17 and `kerr.md` Step 15, and it pins the dot convention down without any algebra.

The checker's dimensional pass does this for every term of every expression, which is 448 components, four geodesic equations, one line element and one scalar, in well under a second.

---

## Step 17. The components, and what the checker needs

In the Boyer-Lindquist chart there are:
...
- eighty eight components of $R^\mu{}_{\nu\rho\sigma}$ and eighty of $R_{\mu\nu\rho\sigma}$, from thirty eight and thirteen distinct values, each written as its Weyl component plus its charge term;
...
That is 448 components, against Kerr's 412; the difference is the thirty six Ricci and Einstein components that a vacuum solution does not have.

Adding the spacetime to the checker took one key in `DIMENSIONS` in `verify_metrics.py`,
...
which is the documented step for adding a spacetime, and which declares the two things the script cannot read off the line element: that $a$ and $r_Q$ are lengths rather than dimensionless numbers, and that $t$ is the coordinate the chart multiplies by $c$.
No key in `PARAMETER_RELATIONS` is needed, because $M$, $a$, $G$ and $r_Q$ are free: every component is an identity for all four, not an identity on a constraint surface.
The extremality condition $a^2 + r_Q^2 \le G^2M^2/c^4$ is a condition for the outer horizon to exist and bound the chart's domain, not a relation the algebra leans on.

Two practical points carry over from Kerr and hold again here.

The index names of every component have to be the coordinate names exactly as the chart spells them, which for Kerr-Newman means `"\\theta"` and `"\\phi"` rather than `"theta"` and `"phi"`.
Getting it wrong is loud in one direction and quiet in the other: `compare_block` reports every misspelled index as a disagreement, but the dimensional pass skips a component whose indices it cannot resolve, so a clean `--dimensions-only` run means nothing on a new spacetime until the sympy comparison has confirmed that the index names resolve.

The reader splits a name it does not know into single letters, so `r_Q` has to be kept clear of the letter before it.
`ar_Q^2` parses as $a \cdot r \cdot {}_{-} \cdot Q^2$ and fails; `a\,r_Q^2` parses as intended, and every component that multiplies $r_Q$ by a preceding symbol carries that thin space.
...
Run as it stands, with `--system kerr_newman/boyer_lindquist`, the dimensional pass finishes in about a second and the sympy pass gets through the metric, the inverse metric and the Christoffel symbols in both index positions and then abandons the Riemann tensor on the budget; everything downstream of Riemann is abandoned with it, since the Ricci, Einstein and Weyl tensors are all built from it.
That is reported as `UNCHECKED` with the reason rather than passed in silence, so nothing is claimed on the script's authority that the script did not establish.

What was established, and what stands behind the components:

- The dimensional pass runs on Kerr-Newman in well under a second with `--dimensions-only`, and every term of all 448 components, the four geodesic equations, the line element and the Kretschmann scalar balances. Since the same pass skips silently over any component whose index names it cannot resolve, that is worth only as much as the sympy pass, which confirms that all 448 resolve.
- `check_system` itself was then run with only `norm` replaced. Everything else was the script: its `Reader`, its metric read off the line element, its Christoffel, Riemann, Ricci, Einstein and Weyl built from that metric, its chart weighting, and its component by component comparison, which also requires every other component to vanish. The replacement makes two changes of representation and no change of mathematics: it reduces $\sin\theta$ and $\cos\theta$ to a canonical rational form modulo $\sin^2+\cos^2=1$ instead of calling `simplify`, and it substitutes $M \to \mu c^2/G$ so that the one length the mass enters through is one symbol, which is a bijective change of a positive parameter and so preserves exactly which expressions are zero.
- On that run the script reported **zero disagreements and zero dimensional failures**, over the metric, the inverse metric, the Christoffel symbols in both index positions, the Riemann tensor in both, the Ricci tensor in all three, the Einstein tensor in all three, the Weyl tensor in both, the Ricci scalar and the four geodesic equations. The whole of it took 364 seconds of work: 81 seconds on $R^\mu{}_{\nu\rho\sigma}$, 60 on $R_{\mu\nu\rho\sigma}$, 88 on $C^\mu{}_{\nu\rho\sigma}$, 58 on $C_{\mu\nu\rho\sigma}$, and seconds or less on everything else.
- The Kretschmann scalar is the one thing that did not finish, exactly as it does not for Kerr. Forming it means raising four indices through a metric that is not diagonal and normalising 256 components on the way, and it ran past twenty minutes without finishing. It was checked instead by exact rational arithmetic: the Kretschmann scalar of Step 12 and the contraction $R_{\mu\nu\rho\sigma}R^{\mu\nu\rho\sigma}$ of the computed Riemann tensor were evaluated at four random rational points, with $\theta$ carried by the Weierstrass parametrisation $\sin\theta = 2u/(1+u^2)$, $\cos\theta = (1-u^2)/(1+u^2)$ so that $\sin^2+\cos^2=1$ holds exactly, and agreed at every one. The same was done for $C_{\mu\nu\rho\sigma}C^{\mu\nu\rho\sigma}$ against $K - 8r_Q^4/\Sigma^4$, which is the decomposition of Step 12, and it agreed at every one as well. Both quantities are rational functions of $(r, a, r_s, r_Q, u)$, so agreement at random points is agreement of the functions unless the difference vanishes on a variety through all of them.
- The electromagnetic field of Step 6 was checked in the same way and symbolically: $dF = 0$ and $\nabla_\mu F^{\mu\nu} = 0$ in all four slots, $F$ equals the combination of $U$ and $V$ of Step 6, and the Ricci tensor of Step 7 equals both $2r_Q^2\left(\hat F_{\mu\alpha}\hat F_\nu{}^\alpha - \tfrac14 g_{\mu\nu}\hat F^2\right)$ and $\mathcal{E}\left(g + 2e^0e^0 - 2e^1e^1\right)$, in every slot.
- The split of Step 9, $R = C + \mathcal{E}\left(U\otimes U + V\otimes V\right)$, was checked in all 256 slots of both index positions, and every Riemann component is written from it.

The honest summary is that the physics is verified and the tool is slower than the physics.
The fix is the same as in `kerr.md` Step 17: giving `norm` a canonical trig reduction to fall back on before it reaches for `simplify` would bring both spacetimes inside the budget, and collapsing $GM/c^2$ to one symbol before the algebra would bring Kerr-Newman inside it comfortably.
...
`norm` now does the canonical trig reduction of `kerr.md` Step 17, with every denominator kept as a product of irreducible factors, and the collapse of $GM/c^2$ turned out not to be needed.
`--system kerr_newman/boyer_lindquist` finishes in about eleven seconds at the default budget and reports no disagreement and nothing unchecked, the Kretschmann scalar included, which is the verdict the replacement run reached.
