...
We work the Tolman-Bondi dust universe in comoving synchronous coordinates.
Every component is derived in order, from the line element down to the geodesic equations, none is left as an exercise, and nothing is asserted that is not computed.

The script `_tools/derivations/verify_metrics.py` repeats every computation in sympy and compares each component with its own result, so the algebra can be checked by hand and by machine independently.
The checker's coverage, and how it came to read the third derivative of Step 6, are in Step 18.

Like the metrics of FRW and Bianchi type I, the Tolman-Bondi metric is not a formula.
Its areal radius $R(r,t)$ and its energy function $E(r)$ are undetermined, and the components impose no field equation on them.
For Tolman-Bondi that matters more than for either of those two.
The solution is named for a dust universe, and the dust is one equation away; the components describe the geometry on both sides of that equation rather than only on the far side of it.
The Einstein tensor gives a density and two pressures in Step 10; setting the pressures to zero gives the evolution equation and the mass function $M(r)$ in Step 11; the time component becomes $\rho = M'/(4\pi R^2\partial_r R)$ in Step 12; and $R = a(t)r$ recovers FRW component for component in Step 13.
...
the convention since 18 September 2026.
It is the contraction the signature asks for, in that it is the one that gives ordinary matter a positive energy density, and that matters in Step 10: the Einstein component $G_{tt}$ is $8\pi G\rho/c^2$ with $\rho$ positive, and on the other contraction every sign of the Ricci and Einstein tensors would turn over and the dust would weigh a negative amount.
...
built from the same contraction, and in Step 14 it is computed from that definition rather than copied from the Riemann tensor.
...
In Schwarzschild the convention is plain: $g_{tt} = -(1-r_s/r)$ against a line element whose time term is $-(1-r_s/r)c^2dt^2$.
Since $t$ here is a time, the chart coordinate is $ct$, and every component with an index $t$ is a component in that chart even though the index is written with the bare letter.
...
We do the whole computation in the chart from the start, as for Bianchi type I and Alcubierre, because the metric functions are undetermined and nothing is lost by differentiating with respect to $x^0$ directly.
Throughout, $\partial_t$ is that derivative and no other:
...
Every factor of $c$ the chart demands is in that one definition, and apart from the $-c^2dt^2$ of the line element no factor of $c$ appears anywhere else.
The energy function is a function of $r$ alone, so its derivative is written with a prime, $E' = dE/dr$.
The relation to the notation a cosmologist uses is $\dot{R} = c\,\partial_t R$, so a chart derivative $\partial_t R$ of $10^{-3}$ is an expansion velocity of $3\times10^5$ metres per second.
...
with $\Gamma$ the Christoffel symbols of the same chart.
That reading is checked term by term in Step 17.

Three abbreviations shorten the algebra, and no component is written with them, since the checker's reader has no way to declare them:

$$f^2 \equiv 1 + 2E, \qquad \mathcal{K} \equiv (\partial_t R)^2 - 2E, \qquad \mathcal{J} \equiv \partial_t R\,\partial_r\partial_t R - E'.$$

$\mathcal{J}$ is one half of $\partial_r\mathcal{K}$, which is the one relation between them, and every component that involves any of the three has it written out.
...
There is no $dt\,dr$ term and no shift of any kind, so the curves $r,\theta,\phi = \text{const}$ are worldlines, and they are geodesics, by Step 16.
The matter, once it enters in Step 11, is at rest in these coordinates for all time, which is what a dust can do and a fluid with pressure cannot.
...
That is what makes $t$ a cosmic time in the cosmologist's sense: every observer riding with the dust can set their clock by it.
...
for two free functions $X$ and $R$, and the mixed Einstein component of that general form, computed in Step 10, is

$$G_{tr} = -\frac{2X}{R}\,\partial_t\!\left(\frac{\partial_r R}{X}\right).$$

So $G_{tr}$ vanishes exactly when $\partial_r R/X$ does not depend on $t$, that is when $X = \partial_r R/f(r)$ for some function $f$ of $r$ alone, and writing $f^2 = 1 + 2E$ gives the Lemaître-Tolman-Bondi line element.
...
What is left free after that is $E(r)$, and what is left to impose is the rest of the field equations, imposed in Step 11.
...
It is a length here only because the chart has to give $dr$ some dimension and $\partial_r R$ is then dimensionless, which is the reading of Step 17 and of the checker's `DIMENSIONS` table; relabelling the shells by any increasing $\tilde{r}(r)$ leaves the line element in the same form with $\tilde{E}(\tilde{r}) = E(r)$, so nothing physical depends on which labelling is chosen.

$E$ is dimensionless and has two readings.
Locally it is the energy per unit mass of the shell $r$ in units of $c^2$: by Step 11 the evolution equation it satisfies is the Newtonian energy equation of a particle falling radially, with $E$ the constant total energy.
Geometrically $1 + 2E$ carries the curvature of the spatial slice at that shell, exactly as $1 - kr^2$ does in FRW, which is why $E = -kr^2/2$ in the homogeneous limit of Step 13.
A shell with $E > 0$ has enough energy to expand forever, a shell with $E < 0$ must turn around and recollapse, and $E = 0$ is the marginally bound case, which the general metric deliberately does not assume.
...
$\partial_r R \to 0$ is a shell crossing, where two labels have reached the same areal radius, and there the density blows up, by Step 12, and the Kretschmann scalar with it, by Step 15.
...
The determinant is nonzero wherever the three regularity conditions of Step 2 hold, and the volume element $\sqrt{-g}\,dr\,d\theta\,d\phi$ is the one integrated in Step 12 to get the mass inside a shell.
...
Counting the symmetric partners $\Gamma^\mu{}_{\nu\rho} = \Gamma^\mu{}_{\rho\nu}$, there are nineteen nonzero symbols with an upper index, from eleven distinct values.
...
That is nineteen more symbols, from seven distinct values, since lowering the index has merged several that differed only by a factor of the metric.
...
The radial metric component already carries one derivative of $R$, and a curvature carries two derivatives of the metric, so the curvature of this line element reaches the third derivative of the function the line element is written with, which is one order past any other metric the checker handles.
What that cost at the checker, until the checker learned to read it, is in Step 18.
...
There are twenty four nonzero components of $R^\mu{}_{\nu\rho\sigma}$ from ten distinct values and twenty four of $R_{\mu\nu\rho\sigma}$ from six, the fully lowered ones having fewer distinct values because lowering the first index merges pairs that the mixed ones keep apart.

---

## Step 8. The Ricci tensor

Contracting on the first lower index, $R_{\mu\nu} = R^\alpha{}_{\mu\alpha\nu}$, each diagonal component is a sum of three Riemann components.
...
The comoving congruence of Step 16 is geodesic and has no rotation, so its expansion is $\Theta = \partial_t\ln\sqrt{-g} = \partial_r\partial_t R/\partial_r R + 2\,\partial_t R/R$, the sum of one radial and two transverse rates, and that expression rearranges to
...
By Step 12 the left hand side is $4\pi G\rho/c^2$, so a positive density drives $\partial_t\Theta$ down and the expansion decelerates.
...
The mixed components all vanish, including $R_{tr}$, whose vanishing carries the argument of Step 10.

The mixed and doubly raised forms are one or two factors of the inverse metric away, and each of the three index positions has four nonzero components.
...
That is not luck.
$G_{tt}$ is the Hamiltonian constraint of the slicing, and a constraint by definition involves only the slice and its first time derivative; here the slice carries $R$ and $\partial_r R$ and its first time derivative carries $\partial_t R$ and $\partial_r\partial_t R$, which is exactly what survives.
...
Each of the three index positions of $G_{\mu\nu}$ has four nonzero components.

---

## Step 10. What the Einstein tensor says: no flux, a density and two pressures

Read through the field equations $G_{\mu\nu} = 8\pi G\,T_{\mu\nu}/c^4$, the four Einstein components are a statement about the matter, and it is worth being careful about which four numbers they are.
...
Calling that function $\sqrt{1+2E}$ gives the Lemaître-Tolman-Bondi line element.
So the radial coefficient of that line element is exactly the statement that no energy crosses a comoving shell, and the matter it can hold is therefore matter that stays where it is put.
...
The density here is a mass density, so that $T_{tt} = \rho c^2$ and $G_{tt} = 8\pi G\rho/c^2$, which is the normalisation used for FRW as well.
...
That is the independence of the shells: the model is a stack of noninteracting Newtonian shells, and its exact solution in general relativity is the same one Newton would have written.
...
None of the three is assumed in any component, and the metric stays general for exactly that reason: specialising to the parabolic case would have bought a closed form for $R$ at the price of a solution that no longer has a bound shell in it.
...
That is the density of the dust, and it is the Einstein component $G_{tt}$ read through the field equations and nothing else.
...
The FRW universe is a spacetime in its own right, and its relation to the inhomogeneous dust is exact.
Set
...
which is exactly the FRW line element in its comoving spherical chart.
Every component of the Tolman-Bondi metric reduces to the corresponding FRW component, which is forced rather than lucky: the substitution turns one line element into the other, and each set of components is the curvature of its own line element and nothing else.
Writing $a' = \partial_t a$ for the chart derivative, as for Bianchi type I,

$$G_{tt} = \frac{3\left(a'^2 + k\right)}{a^2}, \qquad G^r{}_r = G^\theta{}_\theta = -\frac{2a\,a'' + a'^2 + k}{a^2}, \qquad R = \frac{6\left(a\,a'' + a'^2 + k\right)}{a^2},$$

and the first of those is the Friedmann equation with the same positive sign as for FRW, which is a check on the contraction convention of Step 1 as much as on the algebra.
The Kretschmann scalar of Step 15 becomes

$$K = \frac{12a''^2}{a^2} + \frac{12\left(a'^2+k\right)^2}{a^4},$$

which is the Kretschmann scalar of FRW, and the Weyl tensor of Step 14 becomes zero in every one of its twenty four slots, which is the vanishing Weyl tensor of FRW and the statement that a homogeneous isotropic universe is conformally flat.
...
The inhomogeneity of the general case is exactly the failure of $M$ to be proportional to $r^3$ at fixed $a$, or of $E$ to be proportional to $r^2$, and the Weyl tensor measures the first of those two directly, by Step 14.

---

## Step 14. The Weyl tensor, computed rather than copied

This spacetime is not a vacuum, so the Weyl tensor is not the Riemann tensor and it is computed from the definition of Step 1 by removing the traces.
...
The twenty four components in each index position reduce to six and eight distinct values respectively, because so many of them are the same value with a sign.

That the Weyl tensor is not the Riemann tensor is visible in any one slot.
In Step 7 $R_{t\theta t\theta} = -R\,\partial_t^2R$, while $C_{t\theta t\theta} = -\mathcal{W}/(6\,\partial_r R)$, and the two agree only where $\mathcal{W} = 6R\,\partial_r R\,\partial_t^2R$, which on the dust shell of Step 11 is the condition $M' = 0$, that is a region with no matter in it.
...
Here they differ in all twenty four slots wherever the density is nonzero, so the Weyl tensor is what the definition gives rather than a copy of anything.

On the dust shell of Step 11 the scalar takes a physical form.
...
The factors of two on the mixed terms are the two orderings of the same symmetric Christoffel symbol, which the sum counts twice.
...
No pressure gradient exists to push a dust element off that path, and the dust condition of Step 11 is that statement in the language of the field equations.

One first integral survives, and the most familiar one does not.
...
That absence is worth naming: it is the same absence that makes the FRW redshift a stretching of wavelengths, and it is the reason an inhomogeneous void model can imitate an accelerating universe.
...
Both match.
On the other reading of the dots, in which $\dot{t}$ meant $dt/d\lambda$, the second of those two terms would come out short by a factor of $c$ against the first, which is the check the dimensional pass makes on every spacetime.
...
## Step 18. The components, the declaration the checker needs, and the third derivative

In the comoving synchronous chart there are one hundred and seventy three expressions:
...
- four Ricci components in each of three index positions, and the Ricci scalar;
- four Einstein components in each of three index positions;
...
For the checker to read the spacetime, `DIMENSIONS` in `verify_metrics.py` needs one line:
...
The areal radius is a length and the shell label beside it is a length too, which is what makes $\partial_r R$ dimensionless; had $r$ been declared dimensionless instead, as a comoving label reasonably might be, $\partial_r R$ would carry a length and not one component would balance.
The energy function has to be dimensionless because it is added to $1$.

The spacetime needs no line in `PARAMETER_RELATIONS`, and that is the most important thing about its declaration.
Every component is an identity in a free $R(r,t)$ and a free $E(r)$.
The evolution equation of Step 11 is not assumed in any component, and if it were the Einstein tensor would have a single nonzero component instead of four, a claim on a constraint surface that the checker compares against free functions.
The same discipline holds for Alcubierre's drive, whose components never use $\partial_t f = -v_s\partial_x f$: the components are what the geometry says, and the field equations are derived from them in Step 11 rather than fed into them.
The cost is that the Einstein tensor alone describes an anisotropic fluid rather than a dust, and the pressures vanish only once the field equations are imposed.

The index names of every component have to be the coordinate names exactly as the chart spells them, so `"\\theta"` and `"\\phi"` and not `"theta"` and `"phi"`.
...
The third derivative is where the checker first reached its limits.
When the Tolman-Bondi metric was added to the checker, `Reader._declare_parameter` in `verify_metrics.py` declared, for a parameter that is a function of several coordinates, every first partial derivative and every second one and nothing beyond, and `_tools/README.md` said that second derivatives are as far as any curvature tensor reaches.
That was true of every other metric the checker read and is false here, for the reason of Step 6: the radial metric component already carries one derivative of $R$, so a curvature, which is two derivatives of the metric, reaches $\partial_r\partial_t^2R$.
Seventy of the one hundred and seventy three expressions contain that third derivative:

- the four Riemann components in the plane of $t$ and $r$, in each of two index positions, eight in all;
- the $tt$ and $rr$ components of the Ricci tensor in all three index positions, six in all, and the Ricci scalar;
- the $\theta\theta$ and $\phi\phi$ components of the Einstein tensor in all three index positions, six in all;
- all twenty four components of the Weyl tensor in each of two index positions, forty eight in all;
- the Kretschmann scalar.

The script reported each of them `UNCHECKED` rather than passing it in silence, once from the dimensional pass and once from the sympy pass, one hundred and forty lines for seventy expressions, and until 22 September 2026 they were the last expressions of any spacetime it could not read.
They were checked in the meantime by a script outside the repository that patched a third loop into `_declare_parameter` and called the script's own `check_system` on this spacetime, and it disagreed with none of them.

Since 22 September 2026 the reader has no fixed order at all.
`Reader._declare_partials` declares a partial derivative when a component names one, at whatever order it is written, provided the function is declared and every coordinate it is differentiated along is one the function is declared to depend on.
A third loop would have closed the gap for Tolman-Bondi and left the next metric one order further on to meet the same wall, while declaring on demand costs nothing for the orders nobody writes and still turns a typo into an error rather than a new symbol: $\partial_\theta R$ is refused with the reason that $R$ is declared a function of $r$ and $t$ only.
...
All one hundred and seventy three expressions are now compared against sympy in the ordinary way and agree:
...
That the seventy are really compared and not merely parsed was confirmed by breaking them: one run doubled the third derivative term of the Kretschmann scalar and the third derivative in one Weyl component, and came back with exactly those two disagreements, and a second run spelled the third derivative $\partial_t\partial_r\partial_t R$ throughout the Weyl components and came back clean.
